...
def Save(a=""):
    if (not is_num(e2.get())) or float(e2.get()) <= 0:#checks if weight entry isn't a number or float
        e2.focus()#puts the mouse in the weight entry
        v2.set('')# sets nothing in the weight entry
        return

    if (not is_num(e3.get())) or float(e2.get()) <= 0:#checks if height entry isn't a number or float
        e3.focus()#puts the mouse in the height entry
        v3.set('')#sets nothing in the height entry
        return
    d2 = datetime.strptime(e4.get(), '%d/%m/%Y')#strptime() method creates a datetime object from the given string
    if todaydat <= d2:# if today date is less than or equal to date time of now
        showerror('Update', 'Date should be < System date')# shows messagebox 
        return False
    ...
    if quest == True:# if quest = yes then ...
        sq = '''insert into Report (Name, Weight, Height, B_Date) 
...
def Update(a=""):
    if (not is_num(e2.get())) or float(e2.get()) <= 0:#checks if weight entry isn't number or float
        e2.focus()#puts the mouse in the weight entry
        v2.set('')#sets the weight entry string nothing
        return

    if (not is_num(e3.get())) or float(e2.get()) <= 0:#checks if height entry isn't number or float
        e3.focus()#puts the mouse in the height entry
...
def Calcul(a=""):
    ...
    if (is_num(e2.get())) and float(e2.get()) > 0:#checks if weight entry is number and float
    ...
    if (is_num(e3.get())) and float(e3.get()) > 0:#checks if height entry is number and float
...
def Prvs():
    sqs = "SELECT * FROM Report by id_inc"#selects all records from db table ordered by id
    cr.execute(sqs)#executes the current query
    tbl = cr.fetchall()#prints all rows of a query result set and returns a list of tuples
...
sech= Button(win, text = 'Search', fg = 'green', width = 3,command = search)# search button
...
lb11.place(x=450, y=75)
lb16.place(x=480, y=75)
lb17.place(x=550, y=75)
lb18.place(x=580, y=75)

e1.place(x=120, y=80)
e2.place(x=120, y=120)
e3.place(x=120, y=160)
e4.place(x=120, y=200)

bt1.place(x=20, y=270)
bt2.place(x=20, y=310)
bt3.place(x=120, y=310)
bt4.place(x=320, y=270)
bt5.place(x=120, y=270)
bt6.place(x=220, y=270)
bt7.place(x=220, y=310)
bt8.place(x=20, y=350)
bt9.place(x=120, y=350)
btP.place(x=220, y=350)
btN.place(x=320, y=350)
sech.place(x = 285, y = 75)
lb20.place(x=320, y=310)
